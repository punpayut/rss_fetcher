# worker.py (Rate Limit Fix Version)
...
class NewsAggregator:
    ...
    def get_latest_news(self) -> List[NewsItem]:
        all_items = []
        # We can still fetch from RSS feeds concurrently as it's not a single API
...
class AIProcessor:
    ...
    def analyze_news_item(self, news_item: NewsItem) -> Optional[Dict[str, Any]]:
        ...
        prompt = f"""You are a top-tier financial analyst AI...""" # Using the full, correct prompt
...
def main():
    ...
    # 2. Check which news items are already in Firestore
    items_to_process = []
    ...
    # 3. Analyze new items SEQUENTIALLY with a delay to respect API rate limits.
    ...
    for item in items_to_process:
        ...
        # 4. Save result to Firestore if analysis was successful
        if analysis:
        ...
        # 5. Add a delay before the next API call
        # This is crucial to avoid hitting the rate limit of the Groq API's free tier.
        delay_seconds = 2.5
